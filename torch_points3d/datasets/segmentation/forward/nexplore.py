...
import torch_points3d.datasets.segmentation.s3dis as Nexplore


log = logging.getLogger(__name__)


class _ForwardNexplore(torch.utils.data.Dataset):
    """ Dataset to run forward inference on nexplore (S3DIS) kind of data data. Runs on a whole folder.
    Arguments:
...
class ForwardNexploreDataset(BaseDataset):
    def __init__(self, dataset_opt):
        ...
        self.test_dataset = _ForwardNexplore(
            self._data_path, transforms=transforms
        )
    ...
    def class_to_segments(self):
        ...
        for key in self._train_categories:
            classes_to_segment[key] = Nexplore.seg_classes[key]
